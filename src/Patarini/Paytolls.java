...
public class Paytolls {
    ...
    public static void main(String[] args) throws IOException {
        ...
        expr = br.readLine();
        exp = expr.split(" ");
        int start = Integer.parseInt(exp[0]) - 1;
        int end = Integer.parseInt(exp[1]) - 1;
        ...
        for(int i=0; i<rebra; i++){
            expr = br.readLine();
            exp = expr.split(" ");
            int x = Integer.parseInt(exp[0]) - 1;
            int y = Integer.parseInt(exp[1]) - 1;
            float tezina = Float.parseFloat(exp[2]);
            g.adjList[x].setInfo(x);
            g.adjList[y].setInfo(y);
            g.addEdge(x, y, tezina);
        }

        float[] path = g.dijkstra(start);

        System.out.println((int)(path[end]));
